...
const BottomNav = ({ user, role, lang, handleLogout }) => {
  ...
  const handleFloatingButton = () => {
    ...
    if (role === "farmer") return navigate("/farmer-dashboard");
    ...
    <div className="fixed bottom-0 left-0 right-0 flex items-center justify-center z-50 md:hidden">
      ...
      ">
            ...
            >
              <i
                className={`${item.icon} text-xl ${
                  location.pathname === item.to ? "text-green-700 scale-110 active-glow" : "text-gray-500"
                }`}
              ></i>
        ...
        <button
          ...
          className="ripple floating-pulse absolute left-1/2 top-0 transform -translate-x-1/2 -translate-y-1/2 bg-green-700 w-16 h-16 rounded-full text-white shadow-2xl border-4 border-white flex items-center justify-center text-4xl font-light transition-all hover:scale-110 active:scale-90"
